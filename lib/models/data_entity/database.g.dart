    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `CoinEntity` (`id` TEXT, `symbol` TEXT, `name` TEXT, `image` TEXT, `currentPrice` REAL, `marketCap` REAL, `marketCapRank` REAL, `priceChange24h` REAL, `priceChangePercentage24h` REAL, PRIMARY KEY (`id`))');
...
class _$CoinDao extends CoinDao {
  ...
  Future<void> insertCoin(CoinEntity coinEntity) async {
    await _coinEntityInsertionAdapter.insert(
        coinEntity, OnConflictStrategy.abort);
  }

  @override
  Future<void> deleteCoin(CoinEntity coinEntity) async {
    await _coinEntityDeletionAdapter.delete(coinEntity);
  }
}
